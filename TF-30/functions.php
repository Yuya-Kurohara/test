...
function my_archive_title( $title ) {
    ...
    function my_the_post_category( $anchor = true, $id = 0 ) {//$anchorにtrueを代入
        global $post;//ループ外で使用するための宣言
        //引数が渡されなければ投稿IDを見るように設定
        if ( 0 === $id ) {
        $id = $post->ID;
        }
        //なぜ$idという変数で、投稿IDを見るように設定したのかというと、get_the_categoryを使うときに投稿IDが必要だから
        ...
        //hrefの中にカテゴリーページのURLを入れるため、get_category_linkで指定したカテゴリーのURLを取得すると宣言。term_idでカテゴリーのIDを取得し、cat_nameでカテゴリー名を取得する
        } else { //引数がfalseならカテゴリー名のみ出力
        echo esc_html( $this_categories[0]->cat_name );
        }
        }
    }
    // つまり、my_the_post_categoryという関数を使う際に、引数falseを入れればリンク無し。trueを入れればリンク有りで出力するということ。

    function my_the_post_tag( $anchor = true, $id = 0) {
        global $post;
        if ( 0 === $id ) {
            $id = $post->ID;
        }
